      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-2">Manage all your guest bookings, check-ins, and financial records.</p>
          </div>
          <div className="flex gap-2">
            <button className="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-blue-700 transition-colors">
              + New Walk-in Booking
            </button>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
            {['All', 'Active', 'Pending', 'Completed', 'Cancelled'].map(t => (
              <button 
                key={t}
                onClick={() => setFilter(t)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors whitespace-nowrap ${filter === t ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <input type="text" placeholder="Search ID or Guest Name..." className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm" />
            </div>
            <button className="bg-gray-50 border border-gray-200 p-2 rounded-xl text-gray-600 hover:bg-gray-100 transition-colors"><Filter className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Guest Details</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Stay Dates</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Total Amount</th>
                ...
                {filteredReservations.map((res, i) => (
                  <tr key={i} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="py-4 px-6">
                      <span className="font-bold text-gray-900 font-mono text-sm bg-gray-100 px-2 py-1 rounded">{res.id}</span>
                    </td>
                    <td className="py-4 px-6">
                      <div className="font-bold text-gray-900 mb-1">{res.guest}</div>
                    ...
                    <td className="py-4 px-6">
                      <div className="text-sm font-medium text-gray-900 flex items-center gap-2 mb-1">
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center text-sm">
            <span className="text-gray-500 font-medium">Showing {filteredReservations.length} reservations</span>
            <div className="flex gap-2">
              <button className="px-3 py-1 border border-gray-200 rounded-md bg-white text-gray-600 hover:bg-gray-50 font-medium disabled:opacity-50">Previous</button>
              <button className="px-3 py-1 border border-gray-200 rounded-md bg-white text-gray-600 hover:bg-gray-50 font-medium disabled:opacity-50">Next</button>
            </div>
          </div>
        </div>
