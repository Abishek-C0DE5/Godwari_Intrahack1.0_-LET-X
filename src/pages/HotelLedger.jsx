...
export default function HotelLedger() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'completed': return <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-200 border border-gray-300"><CheckCircle className="w-3.5 h-3.5"/> Completed</span>;
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-300 mt-2">Manage all your guest bookings, check-ins, and financial records.</p>
          </div>
          <div className="flex gap-2">
            <button className="bg-blue-700 text-white shadow-md shadow-blue-900/50 shadow-xl px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-blue-800 transition-colors">
              + New Walk-in Booking
            </button>
          </div>
        </div>
        ...
        {/* Toolbar */}
        <div className="bg-white/10 backdrop-blur-md border border-white/20 shadow-xl p-4 rounded-2xl shadow-sm border border-white/10 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
            {['All', 'Active', 'Pending', 'Completed', 'Cancelled'].map(t => (
              <button 
                key={t}
                onClick={() => setFilter(t)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${filter === t ? 'bg-blue-700 text-white shadow-xl shadow-blue-900/50 border border-blue-500/50' : 'text-gray-300 hover:text-white hover:bg-white/10 border border-transparent hover:border-white/20'}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <button className="bg-transparent border border-white/10 p-2 rounded-xl text-gray-400 hover:bg-gray-100 transition-colors"><Filter className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 shadow-xl rounded-3xl shadow-sm border border-white/10 overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-transparent/50 border-b border-white/10">
                  <th className="py-4 px-6 text-xs font-bold text-gray-300 uppercase tracking-wider">Booking ID</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-300 uppercase tracking-wider">Guest Details</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-300 uppercase tracking-wider">Stay Dates</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-300 uppercase tracking-wider">Status</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-300 uppercase tracking-wider text-right">Total Amount</th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-300 uppercase tracking-wider text-center">Actions</th>
                </tr>
              </thead>
                ...
                {filteredReservations.map((res, i) => (
                  <tr key={i} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="py-4 px-6">
                      <span className="font-bold text-white font-mono text-sm bg-white/10 border border-white/20 shadow-sm px-2 py-1 rounded">{res.id}</span>
                    </td>
                    <td className="py-4 px-6">
                      <div className="font-bold text-white mb-1">{res.guest}</div>
          ...
          <div className="p-4 border-t border-white/10 bg-transparent/50 flex justify-between items-center text-sm">
            <span className="text-gray-300 font-medium">Showing {filteredReservations.length} reservations</span>
            <div className="flex gap-2">
              <button className="px-3 py-1 border border-white/10 rounded-md bg-white/10 backdrop-blur-md border border-white/20 shadow-xl text-gray-400 hover:bg-transparent font-medium disabled:opacity-50">Previous</button>
              <button className="px-3 py-1 border border-white/10 rounded-md bg-white/10 backdrop-blur-md border border-white/20 shadow-xl text-gray-400 hover:bg-transparent font-medium disabled:opacity-50">Next</button>
            </div>
          </div>
        </div>
